...
## - phg_22.simg is the singularity image file queried in singularity commands. Everything following is a PHG command, Tassel 5 is the pipeline stored in the singularity image.
...
singularity run phg_22.simg /tassel-5-standalone/run_pipeline.pl -debug -MakeDefaultDirectoryPlugin -workingDir phg -endPlugin
...
singularity run phg_22.simg /tassel-5-standalone/run_pipeline.pl -Xmx100G -debug -configParameters phg/initial_config.txt -MakeInitialPHGDBPipelinePlugin -endPlugin
...
singularity run phg_22.simg /tassel-5-standalone/run_pipeline.pl -configParameters phg/maff_from_anchorwave_config.txt -AssemblyMAFFromAnchorWavePlugin -endPlugin

# Second alignment step taking maff files created previously and outputting to gvcf.
#singularity run phg_22.simg /tassel-5-standalone/run_pipeline.pl -Xmx100G -debug -configParameters phg/maft_to_gvcf_config.txt -MAFToGVCFPlugin -endPlugin
...
singularity exec phg_22.simg /shares/baxter/users/tkosfeld/working_PHG/CreateHaplotypesFromGVCF.groovy -config phg/gvcf_config.txt
