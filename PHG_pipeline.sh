...
## - phg_40.simg is the apptainer image file queried in apptainer commands. Everything following is a PHG command, Tassel 5 is the pipeline stored in the apptainer image.
...
apptainer build phg40.simg docker://maizegenetics/phg:0.0.40

# Creation of default directory structure. Only needs to be run once.
# -workingDir is a path directory specified by user.
apptainer run phg_40.simg /tassel-5-standalone/run_pipeline.pl -debug -MakeDefaultDirectoryPlugin -workingDir phg -endPlugin
...
apptainer run phg_40.simg /tassel-5-standalone/run_pipeline.pl -Xmx100G -debug -configParameters phg/initial_config.txt -MakeInitialPHGDBPipelinePlugin -endPlugin
...
apptainer run phg_40.simg /tassel-5-standalone/run_pipeline.pl -configParameters phg/maff_from_anchorwave_config.txt -AssemblyMAFFromAnchorWavePlugin -endPlugin

# Second alignment step taking maff files created previously and outputting to gvcf.
#apptainer run phg_40.simg /tassel-5-standalone/run_pipeline.pl -Xmx100G -debug -configParameters phg/maft_to_gvcf_config.txt -MAFToGVCFPlugin -endPlugin
...
apptainer exec phg_40.simg /shares/baxter/users/tkosfeld/working_PHG/CreateHaplotypesFromGVCF.groovy -config phg/gvcf_config.txt
