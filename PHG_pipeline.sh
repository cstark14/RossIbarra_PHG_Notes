...
apptainer build phgLatest.simg docker://maizegenetics/phg

# Creation of default directory structure. Only needs to be run once.
# -workingDir is a path directory specified by user.
apptainer run phgLatest.simg /tassel-5-standalone/run_pipeline.pl -debug -MakeDefaultDirectoryPlugin -workingDir phg -endPlugin
...
apptainer run phgLatest.simg /tassel-5-standalone/run_pipeline.pl -Xmx100G -debug -configParameters phg/initial_config.txt -MakeInitialPHGDBPipelinePlugin -endPlugin
...
apptainer run phgLatest.simg /tassel-5-standalone/run_pipeline.pl -configParameters phg/maff_from_anchorwave_config.txt -AssemblyMAFFromAnchorWavePlugin -endPlugin
...
apptainer exec phgLatest.simg /shares/baxter/users/tkosfeld/working_PHG/CreateHaplotypesFromGVCF.groovy -config phg/gvcf_config.txt
